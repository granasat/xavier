...
impl<'a> WgfmuDriver<ProductionWgfmu<'a>> for ProductionWgfmu<'a> {
    #[rustfmt::skip]
    fn new() -> Result<ProductionWgfmu<'a>, Box<dyn std::error::Error>> {
            ...
            match op_mode {
                OperationMode::OperationModeFastIV => {
                    ...
                    for i in 0..measurement_size {
                        ...
                        let ret =
                            (self.get_interpolated_force_value)(chan_id as i32, *time, voltage);
                        get_result(ret)?;
