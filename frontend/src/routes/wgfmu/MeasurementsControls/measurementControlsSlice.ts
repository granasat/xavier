...
import pulseCollectionReducer, {
  initialState as pulseCollectionParamsInitialState,
  updateState as updatePulseCollectionParams,
} from "./Pulse/CollectionControls/collectionControlsSlice"
...
export interface WaveformState {
  ...
  pulse: {
    ...
    collectionParams: PulseCollectionControls
...
const initialState: WaveformState = {
  ...
  pulse: {
    ...
    collectionParams: pulseCollectionParamsInitialState,
    type: "Single",
  },
  // Stdp
  stdp: {
...
export const waveformSlice = createSlice({
  ...
  reducers: {
    ...
    setPoints: (
      state,
      action: PayloadAction<VoltageWaveform | StdpWaveform>
    ) => {
    ...
    setPulseParamsField: (
      ...
      action: PayloadAction<{
        val: PulseControls[keyof PulseControls]
        key: keyof PulseControls
      }>
    ) => {
      if (
        action.payload.key == "nPointsHigh" ||
        action.payload.key == "nPointsLow"
      ) {
        ...
        if (
          math.unit(state.pulse.params.cycleTime).value / totalPoints <
          10e-8
        ) {
          let actualPoints =
            parseInt(state.pulse.params.nPointsHigh) +
            parseInt(state.pulse.params.nPointsLow)

          let maxP = Math.floor(
            math.unit(state.pulse.params.cycleTime).value / 10e-8
          )
    ...
    setPulseType: (state, action: PayloadAction<MeasurementType>) => {
      state.pulse.type = action.payload
    },
    ...
    setStdpParamsField: (
      ...
      action: PayloadAction<{
        val:
          | StdpControls[keyof StdpControls]
          | StdpCollectionControls[keyof StdpCollectionControls]
  ...
  extraReducers: (builder) => {
    ...
    //   console.log('first?')
    //   state.pulse.collectionParams = pulseCollectionReducer(
    //     state.pulse.collectionParams,
    //     action
    //   )
    // })
    builder.addMatcher(
      (action) => action.type.includes("pulseCollectionControls"), // Refer to the name atribute on collectionControlsSlice
      (state, action) => {
        state.pulse.collectionParams = pulseCollectionReducer(
          state.pulse.collectionParams,
          action
...
export const selectWaveform = (state: RootState) =>
  state.waveform.waveform.points
export const selectPulseParams = (state: RootState) =>
  state.waveform.pulse.params
export const selectPulseCollectionParams = (state: RootState) =>
  state.waveform.pulse.collectionParams
export const selectPulseType = (state: RootState) => state.waveform.pulse.type
export const selectStdpParams = (state: RootState) => state.waveform.stdp.params
export const selectStdpCollectionParams = (state: RootState) =>
  state.waveform.stdp.collectionParams
export const selectStdpMeasuredConductance = (state: RootState) =>
  state.waveform.stdp.measuredConductance
...
export const selectMeasurement = (state: RootState) =>
  state.waveform.measurement
export const selectTimeScale = (state: RootState) => state.waveform.timeScale
export const selectControlsPage = (state: RootState) =>
  state.waveform.controlsPage
...
export const fetchMeasurement = createAsyncThunk<
  ...
  number,
  { state: RootState }
>("wgfmu/get-measurement", async (id: number, thunkAPI) => {
    ...
    if (
      ...
      measurement = {
        ...
        data: {
          ...
          iv:
            data.iv.length > DATA_LIMIT
              ? data.iv.slice(0, DATA_LIMIT)
              : data.iv,
        } as StdpMeasurement,
      } as Measurement
    } else {
...
export const waitForMeasurement = createAsyncThunk<
  void,
  number,
  { state: RootState }
>("wgfmu/wait-measurement", async (id: number, thunkAPI) => {
  setTimeout(async () => {
    try {
      ...
      if (
        state.waveform.measurement &&
        state.waveform.measurement.id == id &&
        (state.waveform.measurement.status == "Done" ||
          state.waveform.measurement.status == "Error")
      ) {
