                    ...
                    >
                        {
                            measurements.map((measurement, index) => {
                                return (
                                    <a
                                        href="#"
                                        className="ease-in-out duration-200 hover:bg-neutral-700 text-white block px-4 py-2 text-sm  w-full"
                                        role="menuitem"
                                        tabIndex={-1}
                                        id="menu-item-0"
                                        key={index}
                                        onClick={() => dispatch(fetchMeasurement(measurement.id))}
                                    >
                                        <div
                                            className="pointer-events-none text-white w-full text-justify"
                                        >
                                            <p className="text-justify">
                                                {measurement.category} - {(new Date(measurement.date)).toLocaleString("es-ES")} - {measurement.id}
                                            </p>
                                        </div>
                                    </a>
                                )
